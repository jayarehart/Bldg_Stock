...
def create_dyn_inventory_scenario(mat_flow,scenario_name,include_carb=True,plot_emission=True):
    ...
    counter=0
    for i in mat_flow.iterrows():
        # year = mat_flow.loc[i[0]]
        # compute the regrowth for the i_th year
        regrowth_dimlum_i = - mat_flow.loc[i[0]]['Sum_dimlum_inflow'] * biomass_regrowth_i(years=year_vec, year_label=(i[0]), rot_period=rot_period_dimlum, bio_CO2_coeff=ECC_bio_wood, curve='Richards')
        regrowth_glulam_i = - mat_flow.loc[i[0]]['Sum_engwood_inflow'] * biomass_regrowth_i(years=year_vec, year_label=(i[0]), rot_period=rot_period_glulam, bio_CO2_coeff=ECC_bio_wood, curve='Richards')
    ...
    if include_carb==True:
        ...
        SA_i = perc_surface_exposed * mat_flow['Sum_conc_inflow'] * 10e9 / dens_conc / wall_thickness

        # GOING TO WAIT ON THE CARBONATION MODEL FOR NOW, AND JUST LOOK AT BIOGENIC CARBON UPTAKE
    ...
    ECC_CO2_vec = [ECC_CO2_steel, ECC_CO2_conc, ECC_CO2_engwood, ECC_CO2_dimlum, ECC_CO2_masonry]
    ECC_CH4_vec = [ECC_CH4_steel, ECC_CH4_conc, ECC_CH4_engwood, ECC_CH4_dimlum, ECC_CH4_masonry]

    CO2_flux_cradle_to_gate = mat_flow * ECC_CO2_vec
    CH4_flux_cradle_to_gate = mat_flow * ECC_CH4_vec
    ...
    CO2_flux_cradle_to_gate_total_ext = CO2_flux_cradle_to_gate_total.append(pd.DataFrame(np.zeros((417, 1)))).reset_index(drop=True)
    ...
    CO2_flux_total = CO2_flux_cradle_to_gate_total_ext.squeeze() + total_uptake_dimlum + total_uptake_glulam
    ...
    if plot_emission == True:
        ...
        # plt.title('Annual $ CO_2$ Emissions for Scenario: ' + scenario_name)
        ...
        ax0.set_ylabel('Emissions (Mt $ CO_2 / year $)')
        ax0.legend(loc = 'upper right', fontsize="small");
        # plot the CH4 emisisons
        ax1.plot(CH4_flux_cradle_to_gate.index, CH4_flux_cradle_to_gate['Sum_steel_inflow'] * 1e-9, label = 'Steel')
        ax1.plot(CH4_flux_cradle_to_gate.index, CH4_flux_cradle_to_gate['Sum_conc_inflow'] * 1e-9, label = 'Concrete')
        ax1.plot(CH4_flux_cradle_to_gate.index, CH4_flux_cradle_to_gate['Sum_engwood_inflow'] * 1e-9, label = 'Eng Wood')
        ax1.plot(CH4_flux_cradle_to_gate.index, CH4_flux_cradle_to_gate['Sum_dimlum_inflow'] * 1e-9, label = 'Dim Lumber')
        ax1.plot(CH4_flux_cradle_to_gate.index, CH4_flux_cradle_to_gate['Sum_masonry_inflow'] * 1e-9, label = 'Masonry')
        ax1.set_ylabel('Emissions (Mt $ CH_4 / year $)')
        # plot the CO2 uptake
        ax2.plot(np.linspace(start=2017,stop=2200,num=2200-2017+1), total_uptake_dimlum[0:184] * 1e-9, label = 'Dim Lumber', color='#d62728')
        ax2.plot(np.linspace(start=2017,stop=2200,num=2200-2017+1), total_uptake_glulam[0:184] * 1e-9, label = 'Eng Wood', color='#2ca02c')
...
S1_inventory = create_dyn_inventory_scenario(mat_flow=S1_i_mean,scenario_name='S1',include_carb=False,plot_emission=True)
S2_inventory = create_dyn_inventory_scenario(mat_flow=S2_i_mean,scenario_name='S2',include_carb=False,plot_emission=True)
S3_inventory = create_dyn_inventory_scenario(mat_flow=S3_i_mean,scenario_name='S3',include_carb=False,plot_emission=True)
S4_inventory = create_dyn_inventory_scenario(mat_flow=S4_i_mean,scenario_name='S4',include_carb=False,plot_emission=True)
S5_inventory = create_dyn_inventory_scenario(mat_flow=S5_i_mean,scenario_name='S5',include_carb=False,plot_emission=True)
S6_inventory = create_dyn_inventory_scenario(mat_flow=S6_i_mean,scenario_name='S6',include_carb=False,plot_emission=True)
S7_inventory = create_dyn_inventory_scenario(mat_flow=S7_i_mean,scenario_name='S7',include_carb=False,plot_emission=True)
...
axes[0,0].plot(SUMMARY_mass.index, SUMMARY_mass['S7_CO2'],color='tab:gray', linestyle='solid', label = 'S7 (CH4)')
# plot the CH4 emisisons
...
axes[1,1].legend(loc='lower left', fontsize="small", ncol=2);


plt.savefig('./Figures/DLCA/DLCA_output.png', dpi=240)
