...
# LF wood
kde_LF_wood__Steel = calculate_kernel(data=mi_LF_wood['Steel_kgm2'], dist_type='gau', bandwidth=5, plot=False)
kde_LF_wood__Concrete = calculate_kernel(data=mi_LF_wood['Concrete_kgm2'], dist_type='uni', bandwidth=0.0001, plot=False)
kde_LF_wood__Eng_wood = calculate_kernel(data=mi_LF_wood['Eng_wood_kgm2'], dist_type='gau', bandwidth=5, plot=False)
kde_LF_wood__Dim_lumber = calculate_kernel(data=mi_LF_wood['Dim_lumber_kgm2'], dist_type='gau', bandwidth=7, plot=False)
kde_LF_wood__Masonry = calculate_kernel(data=mi_LF_wood['Masonry_kgm2'], dist_type='uni', bandwidth=0.0001, plot=False)

# Mass Timber
kde_Mass_Timber__Steel = calculate_kernel(data=mi_Mass_Timber['Steel_kgm2'], dist_type='gau', bandwidth=0.5, plot=False)
kde_Mass_Timber__Concrete = calculate_kernel(data=mi_Mass_Timber['Concrete_kgm2'], dist_type='uni', bandwidth=0.0001, plot=False)
kde_Mass_Timber__Eng_wood = calculate_kernel(data=mi_Mass_Timber['Eng_wood_kgm2'], dist_type='gau', bandwidth=3, plot=False)
kde_Mass_Timber__Dim_lumber = calculate_kernel(data=mi_Mass_Timber['Dim_lumber_kgm2'], dist_type='uni', bandwidth=0.0001, plot=False)
kde_Mass_Timber__Masonry = calculate_kernel(data=mi_Mass_Timber['Masonry_kgm2'], dist_type='uni', bandwidth=0.0001, plot=False)

# Steel
kde_Steel__Steel = calculate_kernel(data=mi_Steel['Steel_kgm2'], dist_type='gau', bandwidth=5, plot=False)
kde_Steel__Concrete = calculate_kernel(data=mi_Steel['Concrete_kgm2'], dist_type='gau', bandwidth=3, plot=False)
kde_Steel__Eng_wood = calculate_kernel(data=mi_Steel['Eng_wood_kgm2'], dist_type='gau', bandwidth=5, plot=False)
kde_Steel__Dim_lumber = calculate_kernel(data=mi_Steel['Dim_lumber_kgm2'], dist_type='gau', bandwidth=0.0001, plot=False)
kde_Steel__Masonry = calculate_kernel(data=mi_Steel['Masonry_kgm2'], dist_type='uni', bandwidth=0.0001, plot=False)

# RC
kde_RC__Steel = calculate_kernel(data=mi_RC['Steel_kgm2'], dist_type='gau', bandwidth=1, plot=False)
kde_RC__Concrete = calculate_kernel(data=mi_RC['Concrete_kgm2'], dist_type='uni', bandwidth=20, plot=False)
kde_RC__Eng_wood = calculate_kernel(data=mi_RC['Eng_wood_kgm2'], dist_type='gau', bandwidth=0.0001, plot=False)
kde_RC__Dim_lumber = calculate_kernel(data=mi_RC['Dim_lumber_kgm2'], dist_type='gau', bandwidth=0.0001, plot=False)
kde_RC__Masonry = calculate_kernel(data=mi_RC['Masonry_kgm2'], dist_type='uni', bandwidth=0.0001, plot=False)

# RM and URM
kde_RM__Steel = calculate_kernel(data=mi_RM['Steel_kgm2'], dist_type='uni', bandwidth=2, plot=False)
...
def determine_inflow_outflow_new_bldg(scenario, FA_dsm_SSP=FA_dsm_SSP1, lt=lt_dummy, plot=True, plot_title='SSP1 '):
    ...
    def construction_ea_year(year1=2017, year2=2100, scenario_df=scenario_df, type='linear', plot=False):
        ...
        if type=='richards-curve':
            ...
            def compute_logistic(structural_system, year1_adoption, year2_adoption, growth_rate=0.2, M=2050):
...
def calc_inflow_outflow_stock_mats(area_inflow_2017_2100, area_outflow_2017_2100, area_stock_2017_2100,
    ...
    if calc_stock == True:
        ...
        stock_mat['Mass_Timber_conc_stock'] = area_stock_2017_2100['stock_Mass_Timber'] * \
                                              materials_intensity_df['Mass_Timber']['Concrete_kgm2_mean'] * 1e6 / 1e9
...
if MC_sim == True:
    ...
    for i in range(num_iter):
        ...
        S2_mat_i, S2_mat_o, S2_mat_s = calc_inflow_outflow_stock_mats(
            ...
            materials_intensity_df=make_mi_df_sample_df(),
            print_year=0)
        S2_mat_i_list[i] = S2_mat_i
        S2_mat_o_list[i] = S2_mat_o
        ...
        S3_mat_i, S3_mat_o, S3_mat_s, = calc_inflow_outflow_stock_mats(
            ...
            materials_intensity_df=make_mi_df_sample_df(),
            print_year=0)
        S3_mat_i_list[i] = S3_mat_i
        S3_mat_o_list[i] = S3_mat_o
        ...
        S4_mat_i, S4_mat_o, S4_mat_s, = calc_inflow_outflow_stock_mats(
            ...
            materials_intensity_df=make_mi_df_sample_df(),
            print_year=0)
        S4_mat_i_list[i] = S4_mat_i
        S4_mat_o_list[i] = S4_mat_o
        ...
        S5_mat_i, S5_mat_o, S5_mat_s = calc_inflow_outflow_stock_mats(
            ...
            materials_intensity_df=make_mi_df_sample_df(),
            print_year=0)
        S5_mat_i_list[i] = S5_mat_i
        S5_mat_o_list[i] = S5_mat_o
        S5_mat_s_list[i] = S5_mat_s

        # Scenario 6
        S6_sio_new = determine_inflow_outflow_new_bldg(scenario='S_6', FA_dsm_SSP=FA_dsm_SSP2,
                                                       lt=lt_future,
                                                       plot=False, plot_title='SSP2 ')
        S6_area_i, S6_area_o, S6_area_s, = combine_area_existing_and_new(
            os_existing=os_existing_SSP2,
            sio_new_bldg=S6_sio_new,
            plot=False, plot_title='SSP2 + Moderate Mass Timber Growth')
        S6_mat_i, S6_mat_o, S6_mat_s, = calc_inflow_outflow_stock_mats(
            area_inflow_2017_2100=S6_area_i,
            area_outflow_2017_2100=S6_area_o,
            area_stock_2017_2100=S6_area_s,
            materials_intensity_df=make_mi_df_sample_df(),
            print_year=0)
        S6_mat_i_list[i] = S6_mat_i
        S6_mat_o_list[i] = S6_mat_o
        S6_mat_s_list[i] = S6_mat_s

        # Scenario 7
        S7_sio_new = determine_inflow_outflow_new_bldg(scenario='S_7', FA_dsm_SSP=FA_dsm_SSP2,
                                                       lt=lt_future,
                                                       plot=False, plot_title='SSP2 ')
        S7_area_i, S7_area_o, S7_area_s = combine_area_existing_and_new(
            os_existing=os_existing_SSP2,
            sio_new_bldg=S7_sio_new,
            plot=False, plot_title='SSP2 + Low Mass Timber Growth')
        S7_mat_i, S7_mat_o, S7_mat_s = calc_inflow_outflow_stock_mats(
            area_inflow_2017_2100=S7_area_i,
            area_outflow_2017_2100=S7_area_o,
            area_stock_2017_2100=S7_area_s,
            materials_intensity_df=make_mi_df_sample_df(),
            print_year=0)
        S7_mat_i_list[i] = S7_mat_i
        S7_mat_o_list[i] = S7_mat_o
        S7_mat_s_list[i] = S7_mat_s
    ...
    S1_mat_i_p05 = pd.concat(S1_mat_i_list).groupby(level=1).quantile(q=0.05)
    S1_mat_i_p95 = pd.concat(S1_mat_i_list).groupby(level=1).quantile(q=0.95)
    S1_mat_o_mean = pd.concat(S1_mat_o_list).groupby(level=1).mean()
    S1_mat_o_std = pd.concat(S1_mat_o_list).groupby(level=1).std()
    S1_mat_o_p05 = pd.concat(S1_mat_o_list).groupby(level=1).quantile(q=0.05)
    S1_mat_o_p95 = pd.concat(S1_mat_o_list).groupby(level=1).quantile(q=0.95)
    S1_mat_s_mean = pd.concat(S1_mat_s_list).groupby(level=1).mean()
    S1_mat_s_std = pd.concat(S1_mat_s_list).groupby(level=1).std()
    S1_mat_s_p05 = pd.concat(S1_mat_s_list).groupby(level=1).quantile(q=0.05)
    S1_mat_s_p95 = pd.concat(S1_mat_s_list).groupby(level=1).quantile(q=0.95)

    S2_mat_i_mean = pd.concat(S2_mat_i_list).groupby(level=1).mean()
    S2_mat_i_std = pd.concat(S2_mat_i_list).groupby(level=1).std()
    S2_mat_i_p05 = pd.concat(S2_mat_i_list).groupby(level=1).quantile(q=0.05)
    S2_mat_i_p95 = pd.concat(S2_mat_i_list).groupby(level=1).quantile(q=0.95)
    S2_mat_o_mean = pd.concat(S2_mat_o_list).groupby(level=1).mean()
    S2_mat_o_std = pd.concat(S2_mat_o_list).groupby(level=1).std()
    S2_mat_o_p05 = pd.concat(S2_mat_o_list).groupby(level=1).quantile(q=0.05)
    S2_mat_o_p95 = pd.concat(S2_mat_o_list).groupby(level=1).quantile(q=0.95)
    S2_mat_s_mean = pd.concat(S2_mat_s_list).groupby(level=1).mean()
    S2_mat_s_std = pd.concat(S2_mat_s_list).groupby(level=1).std()
    S2_mat_s_p05 = pd.concat(S2_mat_s_list).groupby(level=1).quantile(q=0.05)
    S2_mat_s_p95 = pd.concat(S2_mat_s_list).groupby(level=1).quantile(q=0.95)

    S3_mat_i_mean = pd.concat(S3_mat_i_list).groupby(level=1).mean()
    S3_mat_i_std = pd.concat(S3_mat_i_list).groupby(level=1).std()
    S3_mat_i_p05 = pd.concat(S3_mat_i_list).groupby(level=1).quantile(q=0.05)
    S3_mat_i_p95 = pd.concat(S3_mat_i_list).groupby(level=1).quantile(q=0.95)
    S3_mat_o_mean = pd.concat(S3_mat_o_list).groupby(level=1).mean()
    S3_mat_o_std = pd.concat(S3_mat_o_list).groupby(level=1).std()
    S3_mat_o_p05 = pd.concat(S3_mat_o_list).groupby(level=1).quantile(q=0.05)
    S3_mat_o_p95 = pd.concat(S3_mat_o_list).groupby(level=1).quantile(q=0.95)
    S3_mat_s_mean = pd.concat(S3_mat_s_list).groupby(level=1).mean()
    S3_mat_s_std = pd.concat(S3_mat_s_list).groupby(level=1).std()
    S3_mat_s_p05 = pd.concat(S3_mat_s_list).groupby(level=1).quantile(q=0.05)
    S3_mat_s_p95 = pd.concat(S3_mat_s_list).groupby(level=1).quantile(q=0.95)

    S4_mat_i_mean = pd.concat(S4_mat_i_list).groupby(level=1).mean()
    S4_mat_i_std = pd.concat(S4_mat_i_list).groupby(level=1).std()
    S4_mat_i_p05 = pd.concat(S4_mat_i_list).groupby(level=1).quantile(q=0.05)
    S4_mat_i_p95 = pd.concat(S4_mat_i_list).groupby(level=1).quantile(q=0.95)
    S4_mat_o_mean = pd.concat(S4_mat_o_list).groupby(level=1).mean()
    S4_mat_o_std = pd.concat(S4_mat_o_list).groupby(level=1).std()
    S4_mat_o_p05 = pd.concat(S4_mat_o_list).groupby(level=1).quantile(q=0.05)
    S4_mat_o_p95 = pd.concat(S4_mat_o_list).groupby(level=1).quantile(q=0.95)
    S4_mat_s_mean = pd.concat(S4_mat_s_list).groupby(level=1).mean()
    S4_mat_s_std = pd.concat(S4_mat_s_list).groupby(level=1).std()
    S4_mat_s_p05 = pd.concat(S4_mat_s_list).groupby(level=1).quantile(q=0.05)
    S4_mat_s_p95 = pd.concat(S4_mat_s_list).groupby(level=1).quantile(q=0.95)

    S5_mat_i_mean = pd.concat(S5_mat_i_list).groupby(level=1).mean()
    S5_mat_i_std = pd.concat(S5_mat_i_list).groupby(level=1).std()
    S5_mat_i_p05 = pd.concat(S5_mat_i_list).groupby(level=1).quantile(q=0.05)
    S5_mat_i_p95 = pd.concat(S5_mat_i_list).groupby(level=1).quantile(q=0.95)
    S5_mat_o_mean = pd.concat(S5_mat_o_list).groupby(level=1).mean()
    S5_mat_o_std = pd.concat(S5_mat_o_list).groupby(level=1).std()
    S5_mat_o_p05 = pd.concat(S5_mat_o_list).groupby(level=1).quantile(q=0.05)
    S5_mat_o_p95 = pd.concat(S5_mat_o_list).groupby(level=1).quantile(q=0.95)
    S5_mat_s_mean = pd.concat(S5_mat_s_list).groupby(level=1).mean()
    S5_mat_s_std = pd.concat(S5_mat_s_list).groupby(level=1).std()
    S5_mat_s_p05 = pd.concat(S5_mat_s_list).groupby(level=1).quantile(q=0.05)
    S5_mat_s_p95 = pd.concat(S5_mat_s_list).groupby(level=1).quantile(q=0.95)

    S6_mat_i_mean = pd.concat(S6_mat_i_list).groupby(level=1).mean()
    S6_mat_i_std = pd.concat(S6_mat_i_list).groupby(level=1).std()
    S6_mat_i_p05 = pd.concat(S6_mat_i_list).groupby(level=1).quantile(q=0.05)
    S6_mat_i_p95 = pd.concat(S6_mat_i_list).groupby(level=1).quantile(q=0.95)
    S6_mat_o_mean = pd.concat(S6_mat_o_list).groupby(level=1).mean()
    S6_mat_o_std = pd.concat(S6_mat_o_list).groupby(level=1).std()
    S6_mat_o_p05 = pd.concat(S6_mat_o_list).groupby(level=1).quantile(q=0.05)
    S6_mat_o_p95 = pd.concat(S6_mat_o_list).groupby(level=1).quantile(q=0.95)
    S6_mat_s_mean = pd.concat(S6_mat_s_list).groupby(level=1).mean()
    S6_mat_s_std = pd.concat(S6_mat_s_list).groupby(level=1).std()
    S6_mat_s_p05 = pd.concat(S6_mat_s_list).groupby(level=1).quantile(q=0.05)
    S6_mat_s_p95 = pd.concat(S6_mat_s_list).groupby(level=1).quantile(q=0.95)

    S7_mat_i_mean = pd.concat(S7_mat_i_list).groupby(level=1).mean()
    S7_mat_i_std = pd.concat(S7_mat_i_list).groupby(level=1).std()
    S7_mat_i_p05 = pd.concat(S6_mat_i_list).groupby(level=1).quantile(q=0.05)
    S7_mat_i_p95 = pd.concat(S6_mat_i_list).groupby(level=1).quantile(q=0.95)
    S7_mat_o_mean = pd.concat(S7_mat_o_list).groupby(level=1).mean()
    S7_mat_o_std = pd.concat(S7_mat_o_list).groupby(level=1).std()
    S7_mat_o_p05 = pd.concat(S7_mat_o_list).groupby(level=1).quantile(q=0.05)
    S7_mat_o_p95 = pd.concat(S7_mat_o_list).groupby(level=1).quantile(q=0.95)
    S7_mat_s_mean = pd.concat(S7_mat_s_list).groupby(level=1).mean()
    S7_mat_s_std = pd.concat(S7_mat_s_list).groupby(level=1).std()
    S7_mat_s_p05 = pd.concat(S7_mat_s_list).groupby(level=1).quantile(q=0.05)
    S7_mat_s_p95 = pd.concat(S7_mat_s_list).groupby(level=1).quantile(q=0.95)
...
def plot_sio_materials(s1_inflow=None, s2_inflow=None, s3_inflow=None, s4_inflow=None, s5_inflow=None, s6_inflow=None, s7_inflow=None,
                       s1_inflow_ci=None, s2_inflow_ci=None, s3_inflow_ci=None, s4_inflow_ci=None, s5_inflow_ci=None, s6_inflow_ci=None, s7_inflow_ci=None,
                       s1_outflow=None, s2_outflow=None, s3_outflow=None, s4_outflow=None, s5_outflow=None,s6_outflow=None,s7_outflow=None,
                       s1_outflow_ci=None, s2_outflow_ci=None, s3_outflow_ci=None, s4_outflow_ci=None, s5_outflow_ci=None,s6_outflow_ci=None, s7_outflow_ci=None,
                       s1_stock=None, s2_stock=None, s3_stock=None, s4_stock=None, s5_stock=None,s6_stock=None, s7_stock=None,
                       s1_stock_ci=None, s2_stock_ci=None, s3_stock_ci=None, s4_stock_ci=None, s5_stock_ci=None,s6_stock_ci=None, s7_stock_ci=None,
                    ...
                    if s5_inflow is not None:
                        ...
                        if plot_error == True:
                            ...
                            axs[2, 0].fill_between(s5_inflow.index,
                                                   ...
                                                   (s5_inflow['Sum_masonry_inflow'] + s5_inflow_ci['Sum_masonry_inflow']),
                                                   color='orange', alpha=.1)
                        if s6_inflow is not None:
                            axs[0, 0].plot(s6_inflow.index, s6_inflow['Sum_steel_inflow'], 'tab:gray')
                            axs[0, 1].plot(s6_inflow.index, s6_inflow['Sum_conc_inflow'], 'tab:gray')
                            axs[1, 0].plot(s6_inflow.index, s6_inflow['Sum_engwood_inflow'], 'tab:gray')
                            axs[1, 1].plot(s6_inflow.index, s6_inflow['Sum_dimlum_inflow'], 'tab:gray')
                            axs[2, 0].plot(s6_inflow.index, s6_inflow['Sum_masonry_inflow'], 'tab:gray')
                            if plot_error == True:
                                axs[0, 0].fill_between(s6_inflow.index,
                                                       (s6_inflow['Sum_steel_inflow'] - s6_inflow_ci[
                                                           'Sum_steel_inflow']),
                                                       (s6_inflow['Sum_steel_inflow'] + s6_inflow_ci[
                                                           'Sum_steel_inflow']),
                                                       color='gray', alpha=.1)
                                axs[0, 1].fill_between(s6_inflow.index,
                                                       (s6_inflow['Sum_conc_inflow'] - s6_inflow_ci[
                                                           'Sum_conc_inflow']),
                                                       (s6_inflow['Sum_conc_inflow'] + s6_inflow_ci[
                                                           'Sum_conc_inflow']),
                                                       color='gray', alpha=.1)
                                axs[1, 0].fill_between(s6_inflow.index,
                                                       (s6_inflow['Sum_engwood_inflow'] - s6_inflow_ci[
                                                           'Sum_engwood_inflow']),
                                                       (s6_inflow['Sum_engwood_inflow'] + s6_inflow_ci[
                                                           'Sum_engwood_inflow']),
                                                       color='gray', alpha=.1)
                                axs[1, 1].fill_between(s6_inflow.index,
                                                       (s6_inflow['Sum_dimlum_inflow'] - s6_inflow_ci[
                                                           'Sum_dimlum_inflow']),
                                                       (s6_inflow['Sum_dimlum_inflow'] + s6_inflow_ci[
                                                           'Sum_dimlum_inflow']),
                                                       color='gray', alpha=.1)
                                axs[2, 0].fill_between(s6_inflow.index,
                                                       (s6_inflow['Sum_masonry_inflow'] - s6_inflow_ci[
                                                           'Sum_masonry_inflow']),
                                                       (s6_inflow['Sum_masonry_inflow'] + s6_inflow_ci[
                                                           'Sum_masonry_inflow']),
                                                       color='gray', alpha=.1)
                            if s7_inflow is not None:
                                    axs[0, 0].plot(s7_inflow.index, s7_inflow['Sum_steel_inflow'], 'tab:olive')
                                    axs[0, 1].plot(s7_inflow.index, s7_inflow['Sum_conc_inflow'], 'tab:olive')
                                    axs[1, 0].plot(s7_inflow.index, s7_inflow['Sum_engwood_inflow'], 'tab:olive')
                                    axs[1, 1].plot(s7_inflow.index, s7_inflow['Sum_dimlum_inflow'], 'tab:olive')
                                    axs[2, 0].plot(s7_inflow.index, s7_inflow['Sum_masonry_inflow'], 'tab:olive')
                                    if plot_error == True:
                                        axs[0, 0].fill_between(s7_inflow.index,
                                                               (s7_inflow['Sum_steel_inflow'] - s7_inflow_ci[
                                                                   'Sum_steel_inflow']),
                                                               (s7_inflow['Sum_steel_inflow'] + s7_inflow_ci[
                                                                   'Sum_steel_inflow']),
                                                               color='olive', alpha=.1)
                                        axs[0, 1].fill_between(s7_inflow.index,
                                                               (s7_inflow['Sum_conc_inflow'] - s7_inflow_ci[
                                                                   'Sum_conc_inflow']),
                                                               (s7_inflow['Sum_conc_inflow'] + s7_inflow_ci[
                                                                   'Sum_conc_inflow']),
                                                               color='olive', alpha=.1)
                                        axs[1, 0].fill_between(s7_inflow.index,
                                                               (s7_inflow['Sum_engwood_inflow'] - s7_inflow_ci[
                                                                   'Sum_engwood_inflow']),
                                                               (s7_inflow['Sum_engwood_inflow'] + s7_inflow_ci[
                                                                   'Sum_engwood_inflow']),
                                                               color='olive', alpha=.1)
                                        axs[1, 1].fill_between(s7_inflow.index,
                                                               (s7_inflow['Sum_dimlum_inflow'] - s7_inflow_ci[
                                                                   'Sum_dimlum_inflow']),
                                                               (s7_inflow['Sum_dimlum_inflow'] + s7_inflow_ci[
                                                                   'Sum_dimlum_inflow']),
                                                               color='olive', alpha=.1)
                                        axs[2, 0].fill_between(s7_inflow.index,
                                                               (s7_inflow['Sum_masonry_inflow'] - s7_inflow_ci[
                                                                   'Sum_masonry_inflow']),
                                                               (s7_inflow['Sum_masonry_inflow'] + s7_inflow_ci[
                                                                   'Sum_masonry_inflow']),
                                                               color='olive', alpha=.1)
            ...
            if s3_outflow is not None:
                ...
                if s4_outflow is not None:
                    ...
                    if s5_outflow is not None:
                        ...
                        if s6_outflow is not None:
                            axs[0, 0].plot(s6_outflow.index, s6_outflow['Sum_steel_outflow'], 'tab:gray')
                            axs[0, 1].plot(s6_outflow.index, s6_outflow['Sum_conc_outflow'], 'tab:gray')
                            axs[1, 0].plot(s6_outflow.index, s6_outflow['Sum_engwood_outflow'], 'tab:gray')
                            axs[1, 1].plot(s6_outflow.index, s6_outflow['Sum_dimlum_outflow'], 'tab:gray')
                            axs[2, 0].plot(s6_outflow.index, s6_outflow['Sum_masonry_outflow'], 'tab:gray')
                            if plot_error == True:
                                axs[0, 0].fill_between(s6_outflow.index,
                                                       (s6_outflow['Sum_steel_outflow'] - s6_outflow_ci[
                                                           'Sum_steel_outflow']),
                                                       (s6_outflow['Sum_steel_outflow'] + s6_outflow_ci[
                                                           'Sum_steel_outflow']),
                                                       color='gray', alpha=.1)
                                axs[0, 1].fill_between(s6_outflow.index,
                                                       (s6_outflow['Sum_conc_outflow'] - s6_outflow_ci[
                                                           'Sum_conc_outflow']),
                                                       (s6_outflow['Sum_conc_outflow'] + s6_outflow_ci[
                                                           'Sum_conc_outflow']),
                                                       color='gray', alpha=.1)
                                axs[1, 0].fill_between(s6_outflow.index,
                                                       (s6_outflow['Sum_engwood_outflow'] - s6_outflow_ci[
                                                           'Sum_engwood_outflow']),
                                                       (s6_outflow['Sum_engwood_outflow'] + s6_outflow_ci[
                                                           'Sum_engwood_outflow']),
                                                       color='gray', alpha=.1)
                                axs[1, 1].fill_between(s6_outflow.index,
                                                       (s6_outflow['Sum_dimlum_outflow'] - s6_outflow_ci[
                                                           'Sum_dimlum_outflow']),
                                                       (s6_outflow['Sum_dimlum_outflow'] + s6_outflow_ci[
                                                           'Sum_dimlum_outflow']),
                                                       color='gray', alpha=.1)
                                axs[2, 0].fill_between(s6_outflow.index,
                                                       (s6_outflow['Sum_masonry_outflow'] - s6_outflow_ci[
                                                           'Sum_masonry_outflow']),
                                                       (s6_outflow['Sum_masonry_outflow'] + s6_outflow_ci[
                                                           'Sum_masonry_outflow']),
                                                       color='gray', alpha=.1)

                            if s7_outflow is not None:
                                axs[0, 0].plot(s7_outflow.index, s7_outflow['Sum_steel_outflow'], 'tab:olive')
                                axs[0, 1].plot(s7_outflow.index, s7_outflow['Sum_conc_outflow'], 'tab:olive')
                                axs[1, 0].plot(s7_outflow.index, s7_outflow['Sum_engwood_outflow'], 'tab:olive')
                                axs[1, 1].plot(s7_outflow.index, s7_outflow['Sum_dimlum_outflow'], 'tab:olive')
                                axs[2, 0].plot(s7_outflow.index, s7_outflow['Sum_masonry_outflow'], 'tab:olive')
                                if plot_error == True:
                                    axs[0, 0].fill_between(s7_outflow.index,
                                                           (s7_outflow['Sum_steel_outflow'] - s7_outflow_ci[
                                                               'Sum_steel_outflow']),
                                                           (s7_outflow['Sum_steel_outflow'] + s7_outflow_ci[
                                                               'Sum_steel_outflow']),
                                                           color='olive', alpha=.1)
                                    axs[0, 1].fill_between(s7_outflow.index,
                                                           (s7_outflow['Sum_conc_outflow'] - s7_outflow_ci[
                                                               'Sum_conc_outflow']),
                                                           (s7_outflow['Sum_conc_outflow'] + s7_outflow_ci[
                                                               'Sum_conc_outflow']),
                                                           color='olive', alpha=.1)
                                    axs[1, 0].fill_between(s7_outflow.index,
                                                           (s7_outflow['Sum_engwood_outflow'] - s7_outflow_ci[
                                                               'Sum_engwood_outflow']),
                                                           (s7_outflow['Sum_engwood_outflow'] + s7_outflow_ci[
                                                               'Sum_engwood_outflow']),
                                                           color='olive', alpha=.1)
                                    axs[1, 1].fill_between(s7_outflow.index,
                                                           (s7_outflow['Sum_dimlum_outflow'] - s7_outflow_ci[
                                                               'Sum_dimlum_outflow']),
                                                           (s7_outflow['Sum_dimlum_outflow'] + s7_outflow_ci[
                                                               'Sum_dimlum_outflow']),
                                                           color='olive', alpha=.1)
                                    axs[2, 0].fill_between(s7_outflow.index,
                                                           (s7_outflow['Sum_masonry_outflow'] - s7_outflow_ci[
                                                               'Sum_masonry_outflow']),
                                                           (s7_outflow['Sum_masonry_outflow'] + s7_outflow_ci[
                                                               'Sum_masonry_outflow']),
                                                           color='olive', alpha=.1)
            ...
            if s3_stock is not None:
                ...
                if s4_stock is not None:
                    ...
                    if s5_stock is not None:
                        ...
                        if s6_stock is not None:
                            axs[0, 0].plot(s6_stock.index, s6_stock['Sum_steel_stock'], 'tab:gray')
                            axs[0, 1].plot(s6_stock.index, s6_stock['Sum_conc_stock'], 'tab:gray')
                            axs[1, 0].plot(s6_stock.index, s6_stock['Sum_engwood_stock'], 'tab:gray')
                            axs[1, 1].plot(s6_stock.index, s6_stock['Sum_dimlum_stock'], 'tab:gray')
                            axs[2, 0].plot(s6_stock.index, s6_stock['Sum_masonry_stock'], 'tab:gray')
                            if plot_error == True:
                                axs[0, 0].fill_between(s6_stock.index,
                                                       (s6_stock['Sum_steel_stock'] - s6_stock_ci['Sum_steel_stock']),
                                                       (s6_stock['Sum_steel_stock'] + s6_stock_ci['Sum_steel_stock']),
                                                       color='gray', alpha=.1)
                                axs[0, 1].fill_between(s6_stock.index,
                                                       (s6_stock['Sum_conc_stock'] - s6_stock_ci['Sum_conc_stock']),
                                                       (s6_stock['Sum_conc_stock'] + s6_stock_ci['Sum_conc_stock']),
                                                       color='gray', alpha=.1)
                                axs[1, 0].fill_between(s6_stock.index,
                                                       (s6_stock['Sum_engwood_stock'] - s6_stock_ci[
                                                           'Sum_engwood_stock']),
                                                       (s6_stock['Sum_engwood_stock'] + s6_stock_ci[
                                                           'Sum_engwood_stock']),
                                                       color='gray', alpha=.1)
                                axs[1, 1].fill_between(s6_stock.index,
                                                       (s6_stock['Sum_dimlum_stock'] - s6_stock_ci['Sum_dimlum_stock']),
                                                       (s6_stock['Sum_dimlum_stock'] + s6_stock_ci['Sum_dimlum_stock']),
                                                       color='gray', alpha=.1)
                                axs[2, 0].fill_between(s6_stock.index,
                                                       (s6_stock['Sum_masonry_stock'] - s6_stock_ci[
                                                           'Sum_masonry_stock']),
                                                       (s6_stock['Sum_masonry_stock'] + s6_stock_ci[
                                                           'Sum_masonry_stock']),
                                                       color='gray', alpha=.1)

                            if s7_stock is not None:
                                axs[0, 0].plot(s7_stock.index, s7_stock['Sum_steel_stock'], 'tab:olive')
                                axs[0, 1].plot(s7_stock.index, s7_stock['Sum_conc_stock'], 'tab:olive')
                                axs[1, 0].plot(s7_stock.index, s7_stock['Sum_engwood_stock'], 'tab:olive')
                                axs[1, 1].plot(s7_stock.index, s7_stock['Sum_dimlum_stock'], 'tab:olive')
                                axs[2, 0].plot(s7_stock.index, s7_stock['Sum_masonry_stock'], 'tab:olive')
                                if plot_error == True:
                                    axs[0, 0].fill_between(s7_stock.index,
                                                           (s7_stock['Sum_steel_stock'] - s7_stock_ci[
                                                               'Sum_steel_stock']),
                                                           (s7_stock['Sum_steel_stock'] + s7_stock_ci[
                                                               'Sum_steel_stock']),
                                                           color='olive', alpha=.1)
                                    axs[0, 1].fill_between(s7_stock.index,
                                                           (s7_stock['Sum_conc_stock'] - s7_stock_ci['Sum_conc_stock']),
                                                           (s7_stock['Sum_conc_stock'] + s7_stock_ci['Sum_conc_stock']),
                                                           color='olive', alpha=.1)
                                    axs[1, 0].fill_between(s7_stock.index,
                                                           (s7_stock['Sum_engwood_stock'] - s7_stock_ci[
                                                               'Sum_engwood_stock']),
                                                           (s7_stock['Sum_engwood_stock'] + s7_stock_ci[
                                                               'Sum_engwood_stock']),
                                                           color='olive', alpha=.1)
                                    axs[1, 1].fill_between(s7_stock.index,
                                                           (s7_stock['Sum_dimlum_stock'] - s7_stock_ci[
                                                               'Sum_dimlum_stock']),
                                                           (s7_stock['Sum_dimlum_stock'] + s7_stock_ci[
                                                               'Sum_dimlum_stock']),
                                                           color='olive', alpha=.1)
                                    axs[2, 0].fill_between(s7_stock.index,
                                                           (s7_stock['Sum_masonry_stock'] - s7_stock_ci[
                                                               'Sum_masonry_stock']),
                                                           (s7_stock['Sum_masonry_stock'] + s7_stock_ci[
                                                               'Sum_masonry_stock']),
                                                           color='olive', alpha=.1)
...
if export_csv == True:
    ...
    S1_mat_i_p05.to_csv('./Results/S1_mat_i_p05.csv', index=True)
    S1_mat_i_p95.to_csv('./Results/S1_mat_i_p95.csv', index=True)
    S1_mat_o_mean.to_csv('./Results/S1_mat_o_mean.csv', index=True)
    S1_mat_o_std.to_csv('./Results/S1_mat_o_std.csv', index=True)
    S1_mat_o_p05.to_csv('./Results/S1_mat_o_p05.csv', index=True)
    S1_mat_o_p95.to_csv('./Results/S1_mat_o_p95.csv', index=True)
    S1_mat_s_mean.to_csv('./Results/S1_mat_s_mean.csv', index=True)
    S1_mat_s_std.to_csv('./Results/S1_mat_s_std.csv', index=True)
    S1_mat_s_p05.to_csv('./Results/S1_mat_s_p05.csv', index=True)
    S1_mat_s_p95.to_csv('./Results/S1_mat_s_p95.csv', index=True)

    S2_mat_i_mean.to_csv('./Results/S2_mat_i_mean.csv', index=True)
    S2_mat_i_std.to_csv('./Results/S2_mat_i_std.csv', index=True)
    S2_mat_i_p05.to_csv('./Results/S2_mat_i_p05.csv', index=True)
    S2_mat_i_p95.to_csv('./Results/S2_mat_i_p95.csv', index=True)
    S2_mat_o_mean.to_csv('./Results/S2_mat_o_mean.csv', index=True)
    S2_mat_o_std.to_csv('./Results/S2_mat_o_std.csv', index=True)
    S2_mat_o_p05.to_csv('./Results/S2_mat_o_p05.csv', index=True)
    S2_mat_o_p95.to_csv('./Results/S2_mat_o_p95.csv', index=True)
    S2_mat_s_mean.to_csv('./Results/S2_mat_s_mean.csv', index=True)
    S2_mat_s_std.to_csv('./Results/S2_mat_s_std.csv', index=True)
    S2_mat_s_p05.to_csv('./Results/S2_mat_s_p05.csv', index=True)
    S2_mat_s_p95.to_csv('./Results/S2_mat_s_p95.csv', index=True)

    S3_mat_i_mean.to_csv('./Results/S3_mat_i_mean.csv', index=True)
    S3_mat_i_std.to_csv('./Results/S3_mat_i_std.csv', index=True)
    S3_mat_i_p05.to_csv('./Results/S3_mat_i_p05.csv', index=True)
    S3_mat_i_p95.to_csv('./Results/S3_mat_i_p95.csv', index=True)
    S3_mat_o_mean.to_csv('./Results/S3_mat_o_mean.csv', index=True)
    S3_mat_o_std.to_csv('./Results/S3_mat_o_std.csv', index=True)
    S3_mat_o_p05.to_csv('./Results/S3_mat_o_p05.csv', index=True)
    S3_mat_o_p95.to_csv('./Results/S3_mat_o_p95.csv', index=True)
    S3_mat_s_mean.to_csv('./Results/S3_mat_s_mean.csv', index=True)
    S3_mat_s_std.to_csv('./Results/S3_mat_s_std.csv', index=True)
    S3_mat_s_p05.to_csv('./Results/S3_mat_s_p05.csv', index=True)
    S3_mat_s_p95.to_csv('./Results/S3_mat_s_p95.csv', index=True)

    S4_mat_i_mean.to_csv('./Results/S4_mat_i_mean.csv', index=True)
    S4_mat_i_std.to_csv('./Results/S4_mat_i_std.csv', index=True)
    S4_mat_i_p05.to_csv('./Results/S4_mat_i_p05.csv', index=True)
    S4_mat_i_p95.to_csv('./Results/S4_mat_i_p95.csv', index=True)
    S4_mat_o_mean.to_csv('./Results/S4_mat_o_mean.csv', index=True)
    S4_mat_o_std.to_csv('./Results/S4_mat_o_std.csv', index=True)
    S4_mat_o_p05.to_csv('./Results/S4_mat_o_p05.csv', index=True)
    ...
    S5_mat_i_p05.to_csv('./Results/S5_mat_i_p05.csv', index=True)
    S5_mat_i_p95.to_csv('./Results/S5_mat_i_p95.csv', index=True)
    S5_mat_o_mean.to_csv('./Results/S5_mat_o_mean.csv', index=True)
    S5_mat_o_std.to_csv('./Results/S5_mat_o_std.csv', index=True)
    S5_mat_o_p05.to_csv('./Results/S5_mat_o_p05.csv', index=True)
    S5_mat_o_p95.to_csv('./Results/S5_mat_o_p95.csv', index=True)
    S5_mat_s_mean.to_csv('./Results/S5_mat_s_mean.csv', index=True)
    S5_mat_s_std.to_csv('./Results/S5_mat_s_std.csv', index=True)
    S5_mat_s_p05.to_csv('./Results/S5_mat_s_p05.csv', index=True)
    S5_mat_s_p95.to_csv('./Results/S5_mat_s_p95.csv', index=True)

    S6_mat_i_mean.to_csv('./Results/S6_mat_i_mean.csv', index=True)
    S6_mat_i_std.to_csv('./Results/S6_mat_i_std.csv', index=True)
    S6_mat_i_p05.to_csv('./Results/S6_mat_i_p05.csv', index=True)
    S6_mat_i_p95.to_csv('./Results/S6_mat_i_p95.csv', index=True)
    S6_mat_o_mean.to_csv('./Results/S6_mat_o_mean.csv', index=True)
    S6_mat_o_std.to_csv('./Results/S6_mat_o_std.csv', index=True)
    S6_mat_o_p05.to_csv('./Results/S6_mat_o_p05.csv', index=True)
    S6_mat_o_p95.to_csv('./Results/S6_mat_o_p95.csv', index=True)
    S6_mat_s_mean.to_csv('./Results/S6_mat_s_mean.csv', index=True)
    S6_mat_s_std.to_csv('./Results/S6_mat_s_std.csv', index=True)
    S6_mat_s_p05.to_csv('./Results/S6_mat_s_p05.csv', index=True)
    S6_mat_s_p95.to_csv('./Results/S6_mat_s_p95.csv', index=True)

    S7_mat_i_mean.to_csv('./Results/S7_mat_i_mean.csv', index=True)
    S7_mat_i_std.to_csv('./Results/S7_mat_i_std.csv', index=True)
    S7_mat_i_p05.to_csv('./Results/S7_mat_i_p05.csv', index=True)
    S7_mat_i_p95.to_csv('./Results/S7_mat_i_p95.csv', index=True)
    S7_mat_o_mean.to_csv('./Results/S7_mat_o_mean.csv', index=True)
    S7_mat_o_std.to_csv('./Results/S7_mat_o_std.csv', index=True)
    S7_mat_o_p05.to_csv('./Results/S7_mat_o_p05.csv', index=True)
    S7_mat_o_p95.to_csv('./Results/S7_mat_o_p95.csv', index=True)
    S7_mat_s_mean.to_csv('./Results/S7_mat_s_mean.csv', index=True)
    S7_mat_s_std.to_csv('./Results/S7_mat_s_std.csv', index=True)
    S7_mat_s_p05.to_csv('./Results/S7_mat_s_p05.csv', index=True)
    S7_mat_s_p95.to_csv('./Results/S7_mat_s_p95.csv', index=True)
...
if load_results == True:
    ...
    S1_mat_o_mean = pd.read_csv('./Results/MFA_results/S1_mat_o_mean.csv')
    S1_mat_o_std = pd.read_csv('./Results/MFA_results/S1_mat_o_std.csv')
    S1_mat_s_mean = pd.read_csv('./Results/MFA_results/S1_mat_s_mean.csv')
    S1_mat_s_std = pd.read_csv('./Results/MFA_results/S1_mat_s_std.csv')

    S2_mat_i_mean = pd.read_csv('./Results/MFA_results/S2_mat_i_mean.csv')
    S2_mat_i_std = pd.read_csv('./Results/MFA_results/S2_mat_i_std.csv')
    S2_mat_o_mean = pd.read_csv('./Results/MFA_results/S2_mat_o_mean.csv')
    S2_mat_o_std = pd.read_csv('./Results/MFA_results/S2_mat_o_std.csv')
    S2_mat_s_mean = pd.read_csv('./Results/MFA_results/S2_mat_s_mean.csv')
    S2_mat_s_std = pd.read_csv('./Results/MFA_results/S2_mat_s_std.csv')

    S3_mat_i_mean = pd.read_csv('./Results/vS3_mat_i_mean.csv')
    S3_mat_i_std = pd.read_csv('./Results/MFA_results/S3_mat_i_std.csv')
    S3_mat_o_mean = pd.read_csv('./Results/MFA_results/S3_mat_o_mean.csv')
    S3_mat_o_std = pd.read_csv('./Results/MFA_results/S3_mat_o_std.csv')
    S3_mat_s_mean = pd.read_csv('./Results/MFA_results/S3_mat_s_mean.csv')
    S3_mat_s_std = pd.read_csv('./Results/MFA_results/S3_mat_s_std.csv')

    S4_mat_i_mean = pd.read_csv('./Results/MFA_results/S4_mat_i_mean.csv')
    S4_mat_i_std = pd.read_csv('./Results/MFA_results/S4_mat_i_std.csv')
    S4_mat_o_mean = pd.read_csv('./Results/MFA_results/S4_mat_o_mean.csv')
    S4_mat_o_std = pd.read_csv('./Results/MFA_results/S4_mat_o_std.csv')
    S4_mat_s_mean = pd.read_csv('./Results/MFA_results/S4_mat_s_mean.csv')
    S4_mat_s_std = pd.read_csv('./Results/MFA_results/S4_mat_s_std.csv')

    S5_mat_i_mean = pd.read_csv('./Results/MFA_results/S5_mat_i_mean.csv')
    S5_mat_i_std = pd.read_csv('./Results/MFA_results/S5_mat_i_std.csv')
    S5_mat_o_mean = pd.read_csv('./Results/MFA_results/S5_mat_o_mean.csv')
    S5_mat_o_std = pd.read_csv('./Results/MFA_results/S5_mat_o_std.csv')
    S5_mat_s_mean = pd.read_csv('./Results/MFA_results/S5_mat_s_mean.csv')
    S5_mat_s_std = pd.read_csv('./Results/MFA_results/S5_mat_s_std.csv')

    S6_mat_i_mean = pd.read_csv('./Results/MFA_results/S6_mat_i_mean.csv')
    S6_mat_i_std = pd.read_csv('./Results/MFA_results/S6_mat_i_std.csv')
    S6_mat_o_mean = pd.read_csv('./Results/MFA_results/S6_mat_o_mean.csv')
    S6_mat_o_std = pd.read_csv('./Results/MFA_results/S6_mat_o_std.csv')
    S6_mat_s_mean = pd.read_csv('./Results/MFA_results/S6_mat_s_mean.csv')
    S6_mat_s_std = pd.read_csv('./Results/MFA_results/S6_mat_s_std.csv')

    S7_mat_i_mean = pd.read_csv('./Results/MFA_results/S7_mat_i_mean.csv')
    S7_mat_i_std = pd.read_csv('./Results/MFA_results/S7_mat_i_std.csv')
    S7_mat_o_mean = pd.read_csv('./Results/MFA_results/S7_mat_o_mean.csv')
    S7_mat_o_std = pd.read_csv('./Results/MFA_results/S7_mat_o_std.csv')
    S7_mat_s_mean = pd.read_csv('./Results/MFA_results/S7_mat_s_mean.csv')
    S7_mat_s_std = pd.read_csv('./Results/MFA_results/S7_mat_s_std.csv')
...
plot_sio_materials(s1_inflow=S1_mat_i_mean, s1_outflow=S1_mat_o_mean, s1_stock=S1_mat_s_mean/1000,
                   ...
                   s2_inflow= S2_mat_i_mean, s2_outflow= S2_mat_o_mean, s2_stock=S2_mat_s_mean/1000,
                   s2_inflow_ci=S2_mat_i_std, s2_outflow_ci=S2_mat_o_std, s2_stock_ci=S2_mat_s_std/1000,
                   s3_inflow=S3_mat_i_mean, s3_outflow=S3_mat_o_mean, s3_stock=S3_mat_s_mean/1000,
                   s3_inflow_ci=S3_mat_i_std, s3_outflow_ci=S3_mat_o_std, s3_stock_ci=S3_mat_s_std/1000,
                   s4_inflow=S4_mat_i_mean, s4_outflow=S4_mat_o_mean, s4_stock=S4_mat_s_mean/1000,
                   s4_inflow_ci=S4_mat_i_std, s4_outflow_ci=S4_mat_o_std, s4_stock_ci=S4_mat_s_std/1000,
                   s5_inflow=S5_mat_i_mean, s5_outflow=S5_mat_o_mean, s5_stock=S5_mat_s_mean/1000,
                   s5_inflow_ci=S5_mat_i_std, s5_outflow_ci=S5_mat_o_std, s5_stock_ci=S5_mat_s_std/1000,
                   s6_inflow=S6_mat_i_mean, s6_outflow=S6_mat_o_mean, s6_stock=S6_mat_s_mean/1000,
                   s6_inflow_ci=S6_mat_i_std, s6_outflow_ci=S6_mat_o_std, s6_stock_ci=S6_mat_s_std/1000,
                   s7_inflow=S7_mat_i_mean, s7_outflow=S7_mat_o_mean, s7_stock=S7_mat_s_mean/1000,
